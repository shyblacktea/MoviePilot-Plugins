      ...
      <div v-if="view === 'history'" class="asa-pg-body">
        ...
        <div v-else class="asa-grid">
          <MediaCard
            ...
            :more-label="t('more')"
            @toggle="toggleOne(item.candidate_id)"
          >
            <template #actions>
              <v-list-item v-if="item.tmdb_url" prepend-icon="mdi-open-in-new" title="TMDB" :href="item.tmdb_url" target="_blank" />
              <v-list-item v-if="item.bangumi_url" prepend-icon="mdi-open-in-new" title="Bangumi" :href="item.bangumi_url" target="_blank" />
              <v-list-item
                v-if="item.status === 'candidate' && item.media_source && item.media_id"
                base-color="primary" prepend-icon="mdi-bell-plus-outline" :title="t('subscribe')"
                :disabled="recognizing.has(item.candidate_id)"
                @click="subscribeCandidate(item)"
              />
              <v-list-item
                base-color="error" prepend-icon="mdi-trash-can-outline" :title="t('delete')"
                @click="askDelete([item.candidate_id])"
              />
            </template>
          </MediaCard>
        </div>
