...
class DownloadDirFilter(_PluginBase):
    # 插件名称
    plugin_name = "下载目录滤镜"
    # 插件描述
    ...
    def init_plugin(self, config: dict):
        """装载配置，并在首次加载时迁移原「路径名称格式化」插件的配置与记录。"""
        self.downloadhis = DownloadHistoryOper()
        self.transferhis = TransferHistoryOper()
        # 停止现有任务
        self.stop_service()
        # 从旧插件迁移配置与处理记录（幂等，仅补缺）
        self.__migrate_legacy_state()
        if not config:
            config = self.get_config()
        self.load_config(config)

    def __migrate_legacy_state(self):
        """从原「路径名称格式化」插件迁移配置与数据，避免重复处理已有种子。"""
        try:
            if not self.get_config():
                legacy_config = self.get_config(plugin_id="FormatDownPath")
                if isinstance(legacy_config, dict) and legacy_config:
                    self.update_config(legacy_config)
                    logger.info("下载目录滤镜：已从路径名称格式化迁移插件配置")
            legacy_records = self.get_data(plugin_id="FormatDownPath") or []
            migrated = 0
            for record in legacy_records:
                if self.get_data(key=record.key) is None:
                    self.save_data(key=record.key, value=record.value)
                    migrated += 1
            if migrated:
                logger.info(f"下载目录滤镜：已从路径名称格式化迁移 {migrated} 条插件数据")
        except Exception as exc:
            logger.warn(f"下载目录滤镜：迁移旧插件状态失败：{exc}")
    ...
    def main(self, downloader: Optional[str] = None, downloadhis: Optional[DownloadHistory] = None,
        ...
        with self._lock:
            ...
            if success and downloadhis:
                # 使用历史记录的识别信息（V3 媒体身份：media_source + media_id）
                meta = MetaInfo(title=downloadhis.torrent_name, subtitle=downloadhis.torrent_description)
                _media_source = None
                if downloadhis.media_source and downloadhis.media_id:
                    try:
                        _media_source = MediaSource(downloadhis.media_source)
                    except ValueError:
                        _media_source = None
                media_info = self.chain.recognize_media(
                    meta=meta, mtype=MediaType(downloadhis.type),
                    media_source=_media_source,
                    media_id=str(downloadhis.media_id) if _media_source else None)
            ...
            if success and not settings.SCRAP_FOLLOW_TMDB \
                    and getattr(media_info, "media_source", None) and getattr(media_info, "media_id", None):
                # V3 媒体身份接口：按 media_source + media_id 查询整理记录
                transfer_history = self.transferhis.get_by_media_identity(
                    media_source=media_info.media_source,
                    media_id=str(media_info.media_id),
                    mtype=media_info.type.value)
                if transfer_history:
