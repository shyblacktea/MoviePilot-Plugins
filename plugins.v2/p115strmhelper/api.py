...
class Api:
    ...
    def get_qrcode_api(params: GetQRCodeParams = Depends()) -> ApiResponse[QRCodeData]:
        ...
        try:
            ...
            qrcode_url = f"https://qrcodeapi.115.com/api/1.0/{final_client_type}/1.0/qrcode?uid={_uid}"
            logger.info(f"【扫码登入】直接请求二维码图片: {qrcode_url}")
            resp = _requests.get(qrcode_url).content
