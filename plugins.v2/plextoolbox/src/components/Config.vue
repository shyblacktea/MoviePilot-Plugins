          ...
          <div class="ptb-workspace">
            <div class="ptb-window">
              ...
              <div v-show="activeTab === 'records'" class="ptb-pane">
                ...
                <VTable v-if="history.length" density="compact" class="ptb-history"><thead><tr><th>时间</th><th>条目</th><th>结果</th></tr></thead><tbody><template v-for="(row, index) in history" :key="index"><tr class="ptb-row" @click="expanded = expanded === index ? -1 : index"><td>{{ fmtTime(row.time || row.created_at) }}</td><td>{{ row.label || '-' }}</td><td>{{ row.written_ok || 0 }} 成功 / {{ row.write_failed || 0 }} 失败</td></tr><tr v-if="expanded === index"><td colspan="3" class="ptb-detail-cell"><div v-for="(item, itemIndex) in row.items || []" :key="itemIndex" class="py-1"><VChip :color="statusColor(item.status)" size="x-small" variant="tonal">{{ statusLabel(item.status) }}</VChip><span class="text-caption ml-2">{{ item.label || item.part_id }}</span></div></td></tr></template></tbody></VTable>
