...
def init_plugin(self, config: dict = None):
    ...
    if config:
        ...
        self._category_movie = config.get("category_movie") or ["电影"]  # 默认为列表形式
...
def _torrents_sort(torrents: Any, dl_type: str):
    ...
    if dl_type == "qbittorrent":
        ...
        torrutes = sorted(torrents, key=lambda x: x.added_date, reverse=False)
    return torrents
...
def _get_trackers(torrent: Any, dl_type: str):
    ...
    try:
        if dl_type == "qbittorrent":
            ...
            tier	整数	跟踪器优先级。较低级别的跟踪器在较高级别的跟踪器之前试用。当特殊条目（如 DHT）不存在时，层号用作占位符时，层号有效
